...
int32_t buildTableScanOperatorParamEx(SOperatorParam** ppRes, SArray* pUidList, int32_t srcOpType, STbNameColMap *pMap, bool tableSeq, STimeWindow *window) {
  *ppRes = taosMemoryMalloc(sizeof(SOperatorParam));
  if (NULL == *ppRes) {
    return terrno;
  }
  ...
  pScan->pColMap->vgId = pMap->vgId;
  tstrncpy(pScan->pColMap->tbName, pMap->tbName, TSDB_TABLE_FNAME_LEN);

  pScan->pColMap->colMap = taosArrayDup(pMap->colMap, NULL);
  if (NULL == pScan->pColMap->colMap) {
    taosMemoryFree(pScan->pColMap);
    taosMemoryFree(pScan);
    taosMemoryFreeClear(*ppRes);
    return terrno;
  }
...
int32_t addSingleExchangeSource(SOperatorInfo* pOperator, SExchangeOperatorBasicParam* pBasicParam) {
  ...
  if (pIdx->inUseIdx < 0) {
    ...
    if (pBasicParam->isVtbRefScan) {
      ...
      dataInfo.colMap = taosMemoryMalloc(sizeof(STbNameColMap));
    ...
    if (pBasicParam->isVtbRefScan) {
      ...
      if (!pDataInfo->colMap) {
        pDataInfo->colMap = taosMemoryMalloc(sizeof(STbNameColMap));
      }
