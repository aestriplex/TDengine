...
int32_t crc32Function(SScalarParam* pInput, int32_t inputNum, SScalarParam* pOutput) {
  ...
  uint32_t *out = (uint32_t *) pOutputData->pData;
  for (int32_t i = 0; i < pInput->numOfRows; ++i) {
    int32_t colId = (pInput->numOfRows == 1) ? 0 : i;
    ...
    char *orgStr = varDataVal(colDataGetData(pInputData, colId));
    int32_t orgLen = varDataLen(colDataGetData(pInputData, colId));
    out[i] = crc32(orgStr, orgLen);
  }
